...
function countSymbols(str) {
    return urlencoded.ucs2decode(str).length;
}
...
function utf8PercentEncode(c) {
    ...
    for (let i = 0; i < buf.length; ++i) {
        str += urlencoded.percentEncode(buf[i]);
    }
...
function parseIPv6(input) {
    ...
    input = urlencoded.ucs2decode(input);
...
function parseHost(input, isNotSpecialArg = false) {
    ...
    const domain = urlencoded.percentDecode(Buffer.from(input)).toString();
...
function parseOpaqueHost(input) {
    ...
    let output = '';
    const decoded = urlencoded.ucs2decode(input);
...
function URLStateMachine(input, base, encodingOverride, url, stateOverride) {
    ...
    this.input = urlencoded.ucs2decode(this.input);
...
URLStateMachine.prototype['parse cannot-be-a-base-URL path'] =
    function parseCannotBeABaseURLPath(c) {
        if (c === 63) {
            ...
            if (!isNaN(c)) {
                this.url.path[0] += percentEncodeChar(
                    c,
    ...
    if (isNaN(c) || (!this.stateOverride && c === 35)) {
        ...
        for (let i = 0; i < buffer.length; ++i) {
            if (
                ...
                this.url.query += urlencoded.percentEncode(buffer[i]);
            } else {
...
export const serializeURL = serializeURL;

export const serializeURLOrigin = function (url) {
...
export const basicURLParse = function (input, options) {
...
export const setTheUsername = function (url, username) {
    url.username = '';
    const decoded = urlencoded.ucs2decode(username);
    for (let i = 0; i < decoded.length; ++i) {
        url.username += percentEncodeChar(decoded[i], isUserinfoPercentEncode);
    }
};

export const setThePassword = function (url, password) {
    url.password = '';
    const decoded = urlencoded.ucs2decode(password);
    for (let i = 0; i < decoded.length; ++i) {
        url.password += percentEncodeChar(decoded[i], isUserinfoPercentEncode);
    }
};

export const serializeHost = serializeHost;

export const cannotHaveAUsernamePasswordPort = cannotHaveAUsernamePasswordPort;


export const serializeInteger = function (integer) {
