...
class RelationAuth {
    ...
    checkUserRelation(context, action, filter) {
         ...
         * @returns
         */
        const checkOnRelationId = (destRelationId, filter) => {
            /**
        ...
        const checkOnMultipleRelations = (relationFilter, intersection, entityFilter) => {
            const getRelationIds = () => {
                ...
                if (relations instanceof Promise) {
                    return relations.then((rs) => rs.map(ele => ele.id));
                }
            ...
            if (relationIds instanceof Promise) {
                return relationIds.then((ids) => {
                    return Promise.all(ids.map(ele => checkOnRelationId(ele, entityFilter))).then((value) => {
                        if (intersection) {
            ...
            const value = relationIds.map(ele => checkOnRelationId(ele, entityFilter));
            if (intersection) {
                return !(value.includes(false));
            }
            return value.includes(true);
