        ...
        for (const attr in projection2) {
            ...
            if (relation === 1 || relation == 0) {
                ...
                if (foreignKey) {
                    ...
                    if (isAggr) {
                        // 是聚合运算，只有后台才需要执行
                        (context instanceof AsyncRowStore_1.AsyncContext) && cascadeSelectionFns.push((result) => {
                            const aggrResults = result.map((row) => {
                                const aggrResult = aggregateFn.call(this, entity2, {
                                    data: subProjection,
                                    filter: (0, filter_1.combineFilters)(entity2, this.getSchema(), [{
                                            [foreignKey]: row.id,
                                        }, subFilter]),
                                    sorter: subSorter,
                                    indexFrom,
                                ...
                                if (aggrResult instanceof Promise) {
                                    return aggrResult.then((aggrResultResult) => Object.assign(row, {
                        ...
                        cascadeSelectionFns.push((result) => {
                            ...
                            const dealWithSubRows = (subRows) => {
                                ...
                                if (result.length == 1) {
                                    ...
                                    result.forEach((ele) => {
                                        const subRowss = subRows.filter(ele2 => ele2[foreignKey] === ele.id);
                        ...
                        cascadeSelectionFns.push((result) => {
                            ...
                            const dealWithSubRows = (subRows) => {
                                ...
                                if (result.length === 1) {
                                    ...
                                    result.forEach((ele) => {
                                        const subRowss = subRows.filter(ele2 => ele2.entityId === ele.id);
                                        (0, assert_1.default)(subRowss);
                                        Object.assign(ele, {
                                            [attr]: subRowss,
                                        });
