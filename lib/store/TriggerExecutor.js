...
class TriggerExecutor {
    ...
    async preCommitTrigger(entity, operation, trigger, context, option) {
        ...
        if (trigger.strict === 'makeSure') {
            ...
            Object.assign(operation.data, {
                ...
                [Entity_1.TriggerTimestampAttribute]: Date.now(),
            });
        }
    }
    ...
    onCommit(trigger, operation, context, option) {
        return async () => {
            await context.begin();
    ...
    async postCommitTrigger(operation, trigger, context, option) {
    ...
    postOperation(entity, operation, context, option, result) {
        ...
        if (triggers) {
            ...
            if (context instanceof SyncRowStore_1.SyncContext) {
                ...
                (0, assert_1.default)(commitTriggers.length === 0, '前台目前应当没有commitTrigger');
            }
            else {
                // 异步context
                ...
                const execCommitTrigger = async (idx) => {
                    ...
                    const trigger = commitTriggers[idx];
                    await this.postCommitTrigger(operation, trigger, context, option);
                    return execCommitTrigger(idx + 1);
                };
    ...
    async checkpoint(context, timestamp) {
        ...
        for (const entity of this.volatileEntities) {
            ...
            for (const row of rows) {
                ...
                await this.onCommit(trigger, operation, context, params)();
            }
        }
        return result;
    }
