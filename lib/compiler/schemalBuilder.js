...
function pushStatementIntoActionAst(moduleName, node, sourceFile) {
    ...
    if (ts.isTypeAliasDeclaration(node) && node.name.text === 'ParticularAction') {
        ...
        if (ts.isUnionTypeNode(type)) {
            actionNames = type.types.map((ele) => {
                (0, assert_1.default)(ts.isTypeReferenceNode(ele));
                const text = ele.typeName.text;
                (0, assert_1.default)(text.endsWith('Action'));
                return (0, utils_1.firstLetterLowerCase)(text.slice(0, text.length - 6));
            });
        }
        else {
            (0, assert_1.default)(ts.isTypeReferenceNode(type));
            const text = type.typeName.text;
            (0, assert_1.default)(text.endsWith('Action'));
            actionNames = [(0, utils_1.firstLetterLowerCase)(text.slice(0, text.length - 6))];
        }
    }
...
function outputAction(outputDir, printer) {
    ...
    for (const entity in ActionAsts) {
        ...
        const actionDiff = (0, lodash_1.difference)(actionNames, actionDefNames);
