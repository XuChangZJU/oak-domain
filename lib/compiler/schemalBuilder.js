...
function analyzeEntity(filename, path, program) {
    ...
    const enumStringAttrs = [];
    const states = [];
    const localEnumStringTypes = [];
...
const initialStatements = () => [
    ...
    factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports([
        ...
        factory.createImportSpecifier(false, undefined, factory.createIdentifier("SelectionHint"))
...
function outputSchema(outputDir, printer) {
    for (const entity in Schema) {
        ...
        const EntityDefAttrs = [
            ...
            factory.createPropertySignature(undefined, factory.createIdentifier("Selection"), undefined, factory.createIntersectionTypeNode([
                factory.createTypeReferenceNode(factory.createIdentifier("Selection"), undefined),
                factory.createTypeLiteralNode([factory.createPropertySignature(undefined, factory.createIdentifier("hint"), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createTypeReferenceNode(factory.createIdentifier("SelectionHint"), undefined))])
            ])),
            ...
            factory.createPropertySignature(undefined, factory.createIdentifier("Update"), undefined, factory.createIntersectionTypeNode([
                factory.createTypeReferenceNode(factory.createIdentifier("UpdateOperation"), undefined),
                factory.createTypeReferenceNode(factory.createIdentifier("SelectionHint"), undefined)
            ])),
            factory.createPropertySignature(undefined, factory.createIdentifier("Remove"), undefined, factory.createIntersectionTypeNode([
                factory.createTypeReferenceNode(factory.createIdentifier("RemoveOperation"), undefined),
                factory.createTypeReferenceNode(factory.createIdentifier("SelectionHint"), undefined)
            ])),
        ...
        if (locale) {
            ...
            properties.forEach((ele) => {
                ...
                const result = printer.printList(ts.ListFormat.SourceFileStatements, factory.createNodeArray([
                    factory.createExportAssignment(undefined, undefined, undefined, ele.initializer)
                ]), sourceFile);
                const filename = path_1.default.join(outputDir, entity, 'locales', `${lng}.ts`);
                (0, fs_1.writeFileSync)(filename, result, { flag: 'w' });
                if (locales[lng]) {
...
function analyzeEntities(inputDir) {
    ...
    const fullFilenames = files.map(ele => {
        ...
        if (env_1.RESERVED_ENTITIES.includes(entity) || env_1.RESERVED_ENTITIES.find(ele2 => entity.startsWith(ele2))) {
