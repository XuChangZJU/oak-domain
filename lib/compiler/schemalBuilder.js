...
const ActionImportStatements = [
    factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports([factory.createImportSpecifier(false, undefined, factory.createIdentifier("ActionDef"))])), factory.createStringLiteral("oak-domain/src/types/Action"), undefined),
    factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports([factory.createImportSpecifier(false, undefined, factory.createIdentifier("GenericAction"))])), factory.createStringLiteral(env_1.ACTION_CONSTANT_IN_OAK_DOMAIN), undefined)
];
const ActionAsts = {};
...
function addActionSource(moduleName, name, node) {
    ...
    (0, assert_1.default)(ts.isStringLiteral(moduleSpecifier) &&
        (moduleSpecifier.text === env_1.ACTION_CONSTANT_IN_OAK_DOMAIN
            || (process.env.IN_OAK_DOMAIN && moduleSpecifier.text === '../actions/action')));
    (0, lodash_1.assign)(ast.importedFrom, {
        [name.text]: env_1.ACTION_CONSTANT_IN_OAK_DOMAIN,
    });
}
function getStringTextFromUnionStringLiterals(moduleName, filename, node, program) {
    ...
    (0, assert_1.default)(ts.isUnionTypeNode(type), `${filename}中引用的action「${name.text}」的定义不是union类型`);
    ...
    if (ts.isImportSpecifier(symbol.getDeclarations()[0])) {
        ...
        (0, assert_1.default)(ts.isTypeAliasDeclaration(symbol.getDeclarations()[0]));
        const ast = ActionAsts[moduleName];
        (0, lodash_1.assign)(ast.importedFrom, {
            [name.text]: 'local',
        });
    }
    const actions = type.types.map(ele => {
...
function dealWithActions(moduleName, filename, node, program, schemaAttrs) {
    const ActionDict = {};
    const actionss = [{
            name: 'GenericAction',
    ...
    if (ts.isUnionTypeNode(node)) {
        ...
        actionss.push(...node.types.map(ele => getStringTextFromUnionStringLiterals(moduleName, filename, ele, program)));
    }
    else {
        (0, assert_1.default)(ts.isTypeReferenceNode(node));
    ...
    actionss.slice(1).forEach(({ name }) => {
        const attr = name.slice(0, 1).toLowerCase().concat(name.slice(1, name.length - 6)).concat('State');
        schemaAttrs.push(factory.createPropertySignature(undefined, factory.createIdentifier(attr), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createTypeReferenceNode(factory.createIdentifier(attr.slice(0, 1).toUpperCase().concat(attr.slice(1))))));
    });
}
function getEntityImported(declaration, filename) {
    ...
    if (ts.isStringLiteral(moduleSpecifier)) {
        ...
        else if (moduleSpecifier.text.startsWith(env_1.ENTITY_PATH_IN_OAK_DOMAIN)) {
            entityImported = moduleSpecifier.text.slice(env_1.ENTITY_PATH_IN_OAK_DOMAIN.length);
        }
    }
    if (entityImported) {
            ...
            declarations.forEach((declaration) => {
                if (declaration.type && ts.isTypeReferenceNode(declaration.type) && ts.isIdentifier(declaration.type.typeName) && declaration.type.typeName.text === 'ActionDef') {
                    ...
                    if (declaration2.getSourceFile() === sourceFile) {
                        pushStatementIntoActionAst(moduleName, declaration2, sourceFile);
                    }
                    ...
                    if (declaration2.getSourceFile() === sourceFile) {
                        ...
                        pushStatementIntoActionAst(moduleName, factory.updateTypeAliasDeclaration(declaration2, declaration2.decorators, [factory.createModifier(ts.SyntaxKind.ExportKeyword)], declaration2.name, declaration2.typeParameters, declaration2.type), sourceFile);
                    }
...
function constructSchema(statements, entity) {
    ...
    const members = [
        // id: String<64>
        factory.createPropertySignature(undefined, factory.createIdentifier('id'), undefined, factory.createTypeReferenceNode(factory.createIdentifier('String'), [
            factory.createLiteralTypeNode(factory.createNumericLiteral(64))
        ])),
        // $$createAt$$: Datetime
        factory.createPropertySignature(undefined, factory.createIdentifier('$$createAt$$'), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createTypeReferenceNode(factory.createIdentifier('Datetime'))),
        // $$updateAt$$: Datetime
        factory.createPropertySignature(undefined, factory.createIdentifier('$$updateAt$$'), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createTypeReferenceNode(factory.createIdentifier('Datetime'))),
        // $$updateAt$$: Datetime
        factory.createPropertySignature(undefined, factory.createIdentifier('$$removeAt$$'), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createTypeReferenceNode(factory.createIdentifier('Datetime')))
    ];
    const members2 = [];
    const { [entity]: manyToOneSet } = ManyToOne;
        ...
        if (ts.isTypeReferenceNode(type)) {
            ...
            if (ts.isIdentifier(typeName)) {
                ...
                if (manyToOneItem) {
                    ...
                    members2.push(factory.createPropertySignature(undefined, name, questionToken, factory.createTypeReferenceNode(createForeignRef(entity, text2, 'Schema'))));
                    const foreignKey = `${attrName}Id`;
                    members.push(factory.createPropertySignature(undefined, factory.createIdentifier(foreignKey), questionToken, factory.createTypeReferenceNode(factory.createIdentifier('String'), [
                        factory.createLiteralTypeNode(factory.createNumericLiteral(64))
                    ])));
                }
                else {
                    ...
                    if (ReversePointerRelations[entity] && attrName === 'entity') {
                        members.push(factory.createPropertySignature(undefined, name, questionToken, factory.createUnionTypeNode(ReversePointerRelations[entity].map(ele => factory.createLiteralTypeNode(factory.createStringLiteral((0, utils_1.firstLetterLowerCase)(ele)))))));
                    }
                    else {
                        members.push(factory.createPropertySignature(undefined, name, questionToken, type));
                    }
                }
            }
            else {
...
function constructFilter(statements, entity) {
                ...
                switch (text) {
                    ...
                    default: {
                        ...
                        if (manyToOneItem) {
                            ...
                            (0, assert_1.default)(text.endsWith('State'));
    ...
    statements.push(factory.createTypeAliasDeclaration(undefined, undefined, factory.createIdentifier('AttrFilter'), ReversePointerRelations[entity] ? [
        factory.createTypeParameterDeclaration(factory.createIdentifier("E"), undefined, factory.createTypeReferenceNode(factory.createIdentifier("Q_EnumValue"), [
            factory.createUnionTypeNode(ReversePointerRelations[entity].map(ele => factory.createLiteralTypeNode(factory.createStringLiteral((0, utils_1.firstLetterLowerCase)(ele)))))
        ]))
...
function constructProjection(statements, entity) {
    ...
    const fnCallNode = factory.createTypeReferenceNode(factory.createIdentifier("ExprOp"), [
...
function constructActions(statements, entity) {
    ...
    let adNodes = [
        manyToOneSet
            ? factory.createTypeReferenceNode(factory.createIdentifier("Omit"), [
                ...
                factory.createUnionTypeNode(manyToOneSet.map((ele) => factory.createLiteralTypeNode(factory.createStringLiteral(`${ele[1]}Id`))))
            ])
            : factory.createTypeReferenceNode(factory.createIdentifier("OpSchema"), undefined)
    ];
    if (manyToOneSet) {
        if (ReversePointerRelations[entity]) {
            ...
            adNodes.push(factory.createUnionTypeNode([
                factory.createTypeLiteralNode([
                    factory.createPropertySignature(undefined, factory.createIdentifier('entity'), undefined, factory.createUnionTypeNode(ReversePointerRelations[entity].map(ele => factory.createLiteralTypeNode(factory.createStringLiteral(`${(0, utils_1.firstLetterLowerCase)(ele)}`))))),
                ...
                ].concat(ReversePointerRelations[entity].map((ele) => factory.createPropertySignature(undefined, factory.createIdentifier(`${(0, utils_1.firstLetterLowerCase)(ele)}`), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createKeywordTypeNode(ts.SyntaxKind.UndefinedKeyword))))),
                factory.createParenthesizedType(factory.createIntersectionTypeNode([
                    ...
                    factory.createTypeReferenceNode(factory.createIdentifier("OneOf"), [
                        factory.createTypeLiteralNode(ReversePointerRelations[entity].map((ele) => factory.createPropertySignature(undefined, factory.createIdentifier(`${(0, utils_1.firstLetterLowerCase)(ele)}`), undefined, factory.createUnionTypeNode([
                            factory.createTypeReferenceNode(createForeignRef(entity, ele, 'CreateSingleOperation'), undefined),
                            factory.createParenthesizedType(factory.createIntersectionTypeNode([
                                factory.createTypeReferenceNode(createForeignRef(entity, ele, 'UpdateOperation'), undefined),
                                factory.createTypeLiteralNode([factory.createPropertySignature(undefined, factory.createIdentifier("id"), undefined, factory.createTypeReferenceNode(factory.createIdentifier("String"), [factory.createLiteralTypeNode(factory.createNumericLiteral("64"))]))])
                            ]))
                        ]))))
                    ])
                ]))
            ...
            adNodes.push(...manyToOneSet.map(([refEntity, attr]) => factory.createParenthesizedType(factory.createUnionTypeNode([
                factory.createTypeLiteralNode([
                    factory.createPropertySignature(undefined, factory.createIdentifier(attr), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createUnionTypeNode([
                        factory.createTypeReferenceNode(createForeignRef(entity, refEntity, 'CreateSingleOperation')),
                        factory.createIntersectionTypeNode([
                            factory.createTypeReferenceNode(createForeignRef(entity, refEntity, 'UpdateOperation'), undefined),
                            factory.createTypeLiteralNode([factory.createPropertySignature(undefined, factory.createIdentifier("id"), undefined, factory.createTypeReferenceNode(factory.createIdentifier("String"), [factory.createLiteralTypeNode(factory.createNumericLiteral("64"))]))])
                        ])
                    ])),
                    factory.createPropertySignature(undefined, factory.createIdentifier(`${attr}Id`), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createKeywordTypeNode(ts.SyntaxKind.UndefinedKeyword))
                ]),
                factory.createTypeLiteralNode([
                    factory.createPropertySignature(undefined, factory.createIdentifier(attr), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createKeywordTypeNode(ts.SyntaxKind.UndefinedKeyword)),
                    factory.createPropertySignature(undefined, factory.createIdentifier(`${attr}Id`), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createTypeReferenceNode(factory.createIdentifier("String"), [factory.createLiteralTypeNode(factory.createNumericLiteral("64"))]))
                ])
            ]))));
        }
    }
        ...
        for (const entityName in foreignKeySet) {
            ...
            foreignKeySet[entityName].forEach((foreignKey) => {
                ...
                propertySignatures.push(factory.createPropertySignature(undefined, factory.createIdentifier(identifier), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createUnionTypeNode([
                    factory.createTypeReferenceNode(createForeignRef(entity, entityName, 'CreateOperation'), undefined),
    ...
    statements.push(factory.createTypeAliasDeclaration(undefined, undefined, factory.createIdentifier("CreateOperationData"), undefined, factory.createIntersectionTypeNode(adNodes)));
    // CreateOperation
    statements.push(factory.createTypeAliasDeclaration(undefined, [factory.createModifier(ts.SyntaxKind.ExportKeyword)], factory.createIdentifier("CreateSingleOperation"), undefined, factory.createTypeReferenceNode(factory.createIdentifier("OakOperation"), [
        ...
        factory.createTypeReferenceNode(factory.createIdentifier("CreateOperationData"), undefined)
        ...
        factory.createLiteralTypeNode(factory.createStringLiteral("create")),
        factory.createTypeReferenceNode(factory.createIdentifier("Array"), [factory.createTypeReferenceNode(factory.createIdentifier("CreateOperationData"), undefined)])
        ...
        factory.createTypeReferenceNode(factory.createIdentifier("CreateSingleOperation"), undefined),
        factory.createTypeReferenceNode(factory.createIdentifier("CreateMultipleOperation"), undefined)
    ])));
    // UpdateOperationData
    adNodes = [
        factory.createTypeReferenceNode(factory.createIdentifier("Partial"), [factory.createTypeReferenceNode(factory.createIdentifier("Omit"), [
                factory.createTypeReferenceNode(factory.createIdentifier("OpSchema"), undefined),
                manyToOneSet ? factory.createUnionTypeNode([
                    factory.createLiteralTypeNode(factory.createStringLiteral("id")),
                ].concat(manyToOneSet.map(ele => factory.createLiteralTypeNode(factory.createStringLiteral(`${ele[1]}Id`))))) : factory.createLiteralTypeNode(factory.createStringLiteral("id")),
            ])])
    ];
    if (manyToOneSet) {
        if (ReversePointerRelations[entity]) {
            ...
            adNodes.push(factory.createUnionTypeNode([
                factory.createTypeLiteralNode([
                    factory.createPropertySignature(undefined, factory.createIdentifier('entity'), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createUnionTypeNode(ReversePointerRelations[entity].map(ele => factory.createLiteralTypeNode(factory.createStringLiteral(`${(0, utils_1.firstLetterLowerCase)(ele)}`))))),
                    factory.createPropertySignature(undefined, factory.createIdentifier('entityId'), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createTypeReferenceNode(factory.createIdentifier("String"), [factory.createLiteralTypeNode(factory.createNumericLiteral("64"))]))
                ].concat(ReversePointerRelations[entity].map((ele) => factory.createPropertySignature(undefined, factory.createIdentifier(`${(0, utils_1.firstLetterLowerCase)(ele)}`), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createKeywordTypeNode(ts.SyntaxKind.UndefinedKeyword))))),
                factory.createParenthesizedType(factory.createIntersectionTypeNode([
                    factory.createTypeLiteralNode([
                        factory.createPropertySignature(undefined, factory.createIdentifier('entity'), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createKeywordTypeNode(ts.SyntaxKind.UndefinedKeyword)),
    ...
    statements.push(factory.createTypeAliasDeclaration(undefined, undefined, factory.createIdentifier("UpdateOperationData"), undefined, factory.createIntersectionTypeNode(adNodes)));
    // UpdateOperation
    statements.push(factory.createTypeAliasDeclaration(undefined, [factory.createModifier(ts.SyntaxKind.ExportKeyword)], factory.createIdentifier("UpdateOperation"), undefined, factory.createTypeReferenceNode(factory.createIdentifier("OakOperation"), [
        ActionAsts[entity] ?
            ...
            factory.createLiteralTypeNode(factory.createStringLiteral("update")),
        factory.createTypeReferenceNode(factory.createIdentifier("UpdateOperationData"), undefined),
        factory.createTypeReferenceNode(factory.createIdentifier("Filter"), undefined)
    ])));
    // RemoveOperationData
    adNodes = [
    ...
    if (manyToOneSet) {
        if (ReversePointerRelations[entity]) {
            adNodes.push(factory.createTypeReferenceNode('OneOf', [
                factory.createTypeLiteralNode(ReversePointerRelations[entity].map((ele) => factory.createPropertySignature(undefined, factory.createIdentifier((0, utils_1.firstLetterLowerCase)(ele)), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createTypeReferenceNode(factory.createIdentifier("Omit"), [
                    factory.createUnionTypeNode([
                        factory.createTypeReferenceNode(createForeignRef(entity, ele, 'UpdateOperation'), undefined),
                        factory.createTypeReferenceNode(createForeignRef(entity, ele, 'RemoveOperation'), undefined)
                    ]),
                    factory.createUnionTypeNode([
                        factory.createLiteralTypeNode(factory.createStringLiteral("id")),
                        factory.createLiteralTypeNode(factory.createStringLiteral("ids")),
            ...
            adNodes.push(factory.createTypeLiteralNode(manyToOneSet.map(([refEntity, attr]) => factory.createPropertySignature(undefined, factory.createIdentifier(attr), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createTypeReferenceNode(factory.createIdentifier("Omit"), [
                factory.createUnionTypeNode([
                    factory.createTypeReferenceNode(createForeignRef(entity, refEntity, 'UpdateOperation'), undefined),
                    factory.createTypeReferenceNode(createForeignRef(entity, refEntity, 'RemoveOperation'), undefined)
                ]),
                factory.createUnionTypeNode([
                    factory.createLiteralTypeNode(factory.createStringLiteral("id")),
                    factory.createLiteralTypeNode(factory.createStringLiteral("ids")),
    ...
    if (oneToManySet) {
        ...
        for (const entityName in foreignKeySet) {
            ...
            foreignKeySet[entityName].forEach((foreignKey) => {
                const identifier = `${entityNameLc}s$${foreignKey}`;
                propertySignatures.push(factory.createPropertySignature(undefined, factory.createIdentifier(identifier), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createTypeReferenceNode(factory.createIdentifier("Omit"), [
                    factory.createUnionTypeNode([
                        factory.createTypeReferenceNode(createForeignRef(entity, entityName, 'UpdateOperation'), undefined),
    ...
    statements.push(factory.createTypeAliasDeclaration(undefined, undefined, factory.createIdentifier("RemoveOperationData"), undefined, factory.createIntersectionTypeNode(adNodes)));
    // RemoveOperation
...
const initialStatements = [
    ...
    ])), factory.createStringLiteral(`${env_1.LIB_OAK_DOMAIN}/src/types/DataType`)),
    /* import {
    ...
    ])), factory.createStringLiteral(`${env_1.LIB_OAK_DOMAIN}/src/types/Demand`)),
    factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports([
    ...
    ])), factory.createStringLiteral(`${env_1.LIB_OAK_DOMAIN}/src/types/Polyfill`)),
    ...
    ])), factory.createStringLiteral("oak-domain/src/types/Entity"), undefined)
];
function outputSubQuery(outputDir, printer) {
    ...
    for (const one in Schema) {
        ...
        ]), undefined), undefined, factory.createToken(ts.SyntaxKind.QuestionToken), factory.createUnionTypeNode(fromEntites.map(ele => factory.createIntersectionTypeNode([
...
function outputEntityDict(outputDir, printer) {
    ...
    statements.push(factory.createTypeAliasDeclaration(undefined, undefined, factory.createIdentifier("ES"), undefined, factory.createTypeLiteralNode(propertySignatures)), factory.createExportAssignment(undefined, undefined, undefined, factory.createIdentifier("ES")));
    const resultFile = ts.createSourceFile("someFileName.ts", "", ts.ScriptTarget.Latest, /*setParentNodes*/ false, ts.ScriptKind.TS);
    const result = printer.printNode(ts.EmitHint.Unspecified, factory.createSourceFile(statements, factory.createToken(ts.SyntaxKind.EndOfFileToken), ts.NodeFlags.None), resultFile);
...
function outputSchema(outputDir, printer) {
    for (const entity in Schema) {
        const statements = (0, lodash_1.cloneDeep)(initialStatements);
        // const { schemaAttrs } = Schema[entity];
        if (ActionAsts[entity]) {
            ...
            for (const a in importedFrom) {
                ...
                if (importedSourceDict.hasOwnProperty(source)) {
                    ...
                    (0, lodash_1.assign)(importedSourceDict, {
                        [source]: [s],
                    });
                }
            }
            ...
            for (const source in importedSourceDict) {
                const namedImports = importedSourceDict[source].map(s => factory.createImportSpecifier(false, undefined, factory.createIdentifier(s)));
                if (source === './Action') {
                    namedImports.push(factory.createImportSpecifier(false, undefined, factory.createIdentifier('ParticularAction')), factory.createImportSpecifier(false, undefined, factory.createIdentifier('Action')));
                    importParticularAction = true;
                }
                statements.push(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports(namedImports)), factory.createStringLiteral(source), undefined));
            }
            if (!importParticularAction) {
                statements.push(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports([
                    factory.createImportSpecifier(false, undefined, factory.createIdentifier('ParticularAction')),
                    factory.createImportSpecifier(false, undefined, factory.createIdentifier('Action'))
                ])), factory.createStringLiteral('./Action'), undefined));
            }
        }
        else {
            statements.push(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports([factory.createImportSpecifier(false, undefined, factory.createIdentifier("GenericAction"))])), factory.createStringLiteral(env_1.ACTION_CONSTANT_IN_OAK_DOMAIN), undefined));
        }
        ...
        for (const k in importedFrom) {
            ...
            if (importedFrom[k] !== 'local') {
                importStatements.push(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports([
                    factory.createImportSpecifier(false, undefined, factory.createIdentifier(k))
                ])), factory.createStringLiteral(importedFrom[k]), undefined));
            }
        }
...
function constructAttributes(entity) {
    ...
    schemaAttrs.forEach((attr) => {
        ...
        result.push(factory.createPropertyAssignment(name, factory.createObjectLiteralExpression(attrAssignments, true)));
    });
    return result;
}
function outputStorage(outputDir, printer) {
    const importStatements = [
        factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports([factory.createImportSpecifier(false, undefined, factory.createIdentifier("StorageSchema"))])), factory.createStringLiteral("oak-domain/src/types/Storage"), undefined)
    ];
    ...
    for (const entity in Schema) {
        ...
        const statements = [
            factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports([factory.createImportSpecifier(false, undefined, factory.createIdentifier("StorageDesc"))])), factory.createStringLiteral("oak-domain/src/types/Storage"), undefined)
        ];
        ...
        statements.push(factory.createVariableStatement([factory.createModifier(ts.SyntaxKind.ExportKeyword)], factory.createVariableDeclarationList([factory.createVariableDeclaration(factory.createIdentifier("desc"), undefined, factory.createTypeReferenceNode(factory.createIdentifier("StorageDesc"), undefined), factory.createObjectLiteralExpression(propertyAssignments, true))], ts.NodeFlags.Const)));
    ...
    importStatements.push(factory.createVariableStatement([factory.createModifier(ts.SyntaxKind.ExportKeyword)], factory.createVariableDeclarationList([factory.createVariableDeclaration(factory.createIdentifier("storageSchema"), undefined, factory.createTypeReferenceNode(factory.createIdentifier("StorageSchema"), undefined), factory.createObjectLiteralExpression(entityAssignments, true))], ts.NodeFlags.Const)));
...
function buildSchema(inputDir, outputDir = env_1.OUTPUT_PATH) {
