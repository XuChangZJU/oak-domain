...
function isEnumType(filename, node) {
    if (ts.isUnionTypeNode(node.type) && ts.isLiteralTypeNode(node.type.types[0])) {
        node.type.types.forEach(function (ele) { return checkStringLiteralLegal(filename, 'attribute', node.name.text, ele); });
        return true;
    }
        ...
        if (ts.isInterfaceDeclaration(node)) {
            ...
            if (node.name.text === 'Schema') {
                ...
                members.forEach(function (attrNode) {
                    var _a, _b;
                    var _c;
                    var _d = attrNode, type = _d.type, name = _d.name, questionToken = _d.questionToken;
                    ...
                    if (ts.isTypeReferenceNode(type)
                        ...
                        if ((referencedSchemas.includes(type.typeName.text) || type.typeName.text === 'Schema')) {
                            ...
                            // 如果是从其它的文件引入的定义类型，在这里也要分析判定成enumStringAttrs
                                ...
                                if (ts.isImportSpecifier(declaration)) {
                                    var typee = checker.getDeclaredTypeOfSymbol(symbol);
                                    var declaration2 = (_c = typee.aliasSymbol) === null || _c === void 0 ? void 0 : _c.getDeclarations()[0];
                    ...
                    if (attrName === 'entity') {
                        ...
                        var _e = tslib_1.__read(typeArguments, 1), node_1 = _e[0];
                    ...
                    if (attrName === 'entityId') {
                        ...
                        var _f = tslib_1.__read(typeArguments, 1), node_2 = _f[0];
