            ...
            if (ts.isIdentifier(typeName)) {
                ...
                if (manyToOneItem) {
                    ...
                    if (ReversePointerRelations[entity] && attrName === 'entity') {
                        ...
                        members.push(factory.createPropertySignature(undefined, name, questionToken, factory.createUnionTypeNode(entityUnionTypeNode)));
                    }
                    else {
...
function constructActions(statements, entity) {
    ...
    let adNodes = [
        manyToOneSet
            ? factory.createTypeReferenceNode(factory.createIdentifier("Omit"), [
                ...
                factory.createUnionTypeNode(!ReversePointerRelations[entity] ? manyToOneSet.map((ele) => factory.createLiteralTypeNode(factory.createStringLiteral(`${ele[1]}Id`))).concat(manyToOneSet.map((ele) => factory.createLiteralTypeNode(factory.createStringLiteral(ele[1])))) : [
    ...
    if (manyToOneSet) {
        if (ReversePointerRelations[entity]) {
            ...
            const manyToOnePropertySignatures = ReversePointerRelations[entity].map((ele) => factory.createPropertySignature(undefined, factory.createIdentifier(`${(0, utils_1.firstLetterLowerCase)(ele)}`), undefined, factory.createUnionTypeNode([
            ...
            adNodes.push(factory.createUnionTypeNode([
                factory.createTypeLiteralNode([
                    factory.createPropertySignature(undefined, factory.createIdentifier('entity'), undefined, factory.createUnionTypeNode(entityUnionTypeNode)),
                ...
                factory.createParenthesizedType(factory.createIntersectionTypeNode([
                    ...
                    factory.createTypeReferenceNode(factory.createIdentifier("OneOf"), [
                        factory.createTypeLiteralNode(manyToOnePropertySignatures)
                    ])
                ]))
            ]));
        }
    ...
    statements.push(factory.createTypeAliasDeclaration(undefined, undefined, factory.createIdentifier("CreateOperationData"), undefined, factory.createTypeReferenceNode(factory.createIdentifier("FormCreateData"), [
    ...
    adNodes = [
        factory.createTypeReferenceNode(factory.createIdentifier("FormUpdateData"), [
            manyToOneSet ? factory.createTypeReferenceNode(factory.createIdentifier("Omit"), [
                factory.createTypeReferenceNode(factory.createIdentifier("OpSchema"), undefined),
                factory.createUnionTypeNode(!ReversePointerRelations[entity] ? manyToOneSet.map((ele) => factory.createLiteralTypeNode(factory.createStringLiteral(`${ele[1]}Id`))).concat(manyToOneSet.map((ele) => factory.createLiteralTypeNode(factory.createStringLiteral(ele[1])))) : [
                    factory.createLiteralTypeNode(factory.createStringLiteral('entity')),
                    factory.createLiteralTypeNode(factory.createStringLiteral('entityId'))
    ...
    if (manyToOneSet) {
        if (ReversePointerRelations[entity]) {
            ...
            adNodes.push(factory.createUnionTypeNode([
                factory.createTypeLiteralNode([
                    factory.createPropertySignature(undefined, factory.createIdentifier('entity'), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createUnionTypeNode(entityUnionTypeNode)),
    ...
    if (manyToOneSet) {
        if (ReversePointerRelations[entity]) {
            const manyToOnePropertySignatures = ReversePointerRelations[entity].map((ele) => factory.createPropertySignature(undefined, factory.createIdentifier((0, utils_1.firstLetterLowerCase)(ele)), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createTypeReferenceNode(factory.createIdentifier("Omit"), [
                factory.createUnionTypeNode([
                    factory.createTypeReferenceNode(createForeignRef(entity, ele, 'UpdateOperation'), undefined),
    ...
    const propertySignatures3 = [];
    if (process.env.TARGET_IN_OAK_DOMAIN) {
        propertySignatures3.push(factory.createIndexSignature(undefined, undefined, [factory.createParameterDeclaration(undefined, undefined, undefined, factory.createIdentifier("k"), undefined, factory.createKeywordTypeNode(ts.SyntaxKind.StringKeyword), undefined)], factory.createKeywordTypeNode(ts.SyntaxKind.AnyKeyword)));
    }
    if (oneToManySet) {
        for (const entityName in foreignKeySet) {
            const entityNameLc = (0, utils_1.firstLetterLowerCase)(entityName);
            foreignKeySet[entityName].forEach((foreignKey) => {
                const identifier = `${entityNameLc}s$${foreignKey}`;
                propertySignatures3.push(factory.createPropertySignature(undefined, factory.createIdentifier(identifier), factory.createToken(ts.SyntaxKind.QuestionToken), factory.createTypeReferenceNode(factory.createIdentifier("Omit"), [
                    factory.createUnionTypeNode([
                        factory.createTypeReferenceNode(createForeignRef(entity, entityName, 'UpdateOperation'), undefined),
    ...
    if (propertySignatures3.length > 0) {
        adNodes.push(factory.createTypeLiteralNode(propertySignatures3));
    }
...
function outputSubQuery(outputDir, printer) {
    const statements = [];
    if (process.env.TARGET_IN_OAK_DOMAIN) {
        statements.push(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports([factory.createImportSpecifier(false, undefined, factory.createIdentifier("Selection"))])), factory.createStringLiteral("../types/Entity"), undefined));
    }
