    ...
    ts.forEachChild(sourceFile, (node) => {
        ...
        if (ts.isInterfaceDeclaration(node)) {
            ...
            if (node.name.text === 'Schema') {
                ...
                isFileCarrier = 'FileCarrierEntityShape' === heritageClauses[0].types[0].expression.text;
...
function outputSchema(outputDir, printer) {
    for (const entity in Schema) {
        ...
        if (isFileCarrier) {
            EntityDefAttrs.push(factory.createPropertySignature(undefined, factory.createIdentifier("IsFileCarrier"), undefined, factory.createLiteralTypeNode(factory.createTrue())));
        }
        if (ActionAsts[entity]) {
